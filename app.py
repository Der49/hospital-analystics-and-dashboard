...
st.caption("Filters below apply to every tab.")

# ============================================================
# FILTER BAR — replaces the sidebar entirely
# ============================================================
...
with f2:
    with st.popover("🏢 Department", use_container_width=True):
...
with f3:
    with st.popover("💳 Insurance", use_container_width=True):
...
with f4:
    with st.popover("🧍 Age Group", use_container_width=True):
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["📊 Overview", "🏢 Operations", "💰 Revenue", "🔁 Readmission", "🧍 Demographics"]
)
...
with tab1:
    ...
    k4.metric("Avg Charge / Stay", f"IDR {avg_charge/1e6:.1f}M")
    k5.metric("30-Day Readmission Rate", f"{readmission_rate:.1%}",
...
with tab3:
    ...
    fig_treemap = px.treemap(diagnosis_revenue, path=["diagnosis"], values="total_revenue",
                              title="Revenue Share by Diagnosis", color="total_revenue",
...
with tab4:
    ...
    worst_dept_name, worst_dept_rate = worst_dept_row["department"], worst_dept_row["readmission_rate"]
    ...
    st.info(
        f"The hospital's overall 30-day readmission rate is **{readmission_rate:.1%}**, compared to a "
        f"**15% benchmark**. **{worst_dept_name}** shows the highest departmental rate at "
        f"**{worst_dept_rate:.1%}**. Readmitted patients show a **{los_comparison}** median length of "
        f"stay ({los_yes:.0f} days) than non-readmitted patients ({los_no:.0f} days). This is estimated "
        f"to represent **IDR {estimated_excess_cost/1e6:,.0f}M** in excess costs relative to benchmark performance."
    )


# ============================================================
# TAB 5: DEMOGRAPHICS & RISK FACTORS
# ============================================================
with tab5:
    ...
    st.caption(
        ...
        "patient age and diagnosis. This is a known limitation of the underlying data and is "
        "disclosed here for analytical transparency."
    )